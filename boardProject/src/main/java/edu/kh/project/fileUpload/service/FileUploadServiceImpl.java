...
// @Transactional
// - Unchecked Exception 발생시 Rollback 수행

// @Transactional(rollbackFor = Exception.class)
// - Exception 이하 예외 발생시 Rollback 수행
//	 == Checked, Unchecked 가리지 않고 예외 발생시 롤백
...
//	if(uploadFile.getOriginalFilename().equals(""));
		if(uploadFile.isEmpty()) { // 위에 2개와 같은 뜻이지만 안정성은 좋은 3번째꺼를 사용하라 
	...
	public String test3(MultipartFile uploadFile) {
		...
		// 2) 파일명 변경하기 // 서버에 파일명이 같은애들이 있으면 자동으로 덮어씌워지기 때문에
		String rename = FileUtil.rename(uploadFile.getOriginalFilename());
		...
		try {
			...
			//		메서드 내부에서 예외가 처리되어
			//		메서드 종료시 예외가 던져지지 않아
			//		@Transcational이 rollback을 수행할 수 없게된다
