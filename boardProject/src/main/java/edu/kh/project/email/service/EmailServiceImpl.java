...
public class EmailServiceImpl implements EmailService{
	...
	public int sendEmail(String htmlName, String email) {
		...
		try {
			...
			// 이메일 발송시 사용할 html 파일의 이름에따라
			// 이메일 제목, 내용 다르게 설정
			switch(htmlName) {
				case "signUp" : emailTitle = "[boardProject] 회원 가입 인증번호 입니다.";
				break;
				
				case "findPw" : emailTitle = "[boardProject] 비밀번호 찾기 인증번호 입니다";
				break;
			}
				...
				// MimeMessageHelper : Spring에서 제공하는 메일 발송 도우미
				...
				// 매개변수1 : 이메일 내용
				// 매개변수2 : HTML 코드 해석 여부 지정
				
				// 지정된 HTML 파일에 authKey가 첨부된 후
				// HTML 코드 전체가 하나의 String으로 변환되서 반환 받은후
				// 변환된 String을 메일 내용으로 세팅
				
				// CID(Content-ID)를 이용해 메일에 이미지 처부
				helper.addInline("logo", new ClassPathResource("static/images/logo.gif"));
