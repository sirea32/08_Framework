...
@Transactional // 서비스 내 메서드 수행중 
									// UncheckedException 발생시 rollback
...
public class MyPageServiceImpl implements MyPageService{
	...
	private MyPageMapper mapper; // <= mapper 에 의존성 주입
	...
	public int updateInfo(Member inputMember) {
		...
		if(inputMember.getMemberAddress().equals(",,")) {
			...
			// UPDATE 구문 수행시 MEMBER_ADDRESS 컬럼 값이 NULL이됨
		}
	...
	public int changePw(String currentPw, String newPw, Member loginMember) {
		...
		//		(BCryptPassworedEncoder.matches(평문, 암호문) 이용)
	...
	public String profile(MultipartFile profileImg, int memberNo) {
		...
		try {
			...
			// 업로드되어 임시저장된 이미지를 지정된 경로에 옮기는 작업
