...
memberNickname?.addEventListener("input", ()=>{
  ...
  // 조건이 하나라도 false인 경우
  if( (lengthCheck && validCharactersCheck) === false ){ // 동일 ( !(lengthCheck && validCharactersCheck) )
    memberNickname.classList.remove("green");
    ...
    // 비동기로 입력된 닉네임이 DB에 존재하는지 확인하는 
    // Ajax 코드 작성(fetch() API) 작성   (중복검사)

    // get방식 요청         post방식 요청
    // fetch("요청주소")    fetch("요청주소", {})

    // get방식 요청(쿼리스트링으로 파라미터 전달)
    fetch("/myPage/checkNickname?input=" + inputValue)
    .then(response =>{
        if(response.ok){ // 응답 상태코드 200(성공)인 경우
            return response.text(); // 응답결과를 text형태로 변환
        }
        throw new Error("쭝뽁 겸상 띨때 : " + response.status);
    })
    .then(result =>{
...
updateInfo?.addEventListener("submit", e=>{
  ...
  // for ~ in 구문 : JS 객체의 key값을 하나씩 접근하는 반복문
...
// "변경하기" 버튼 클릭시 또는 form 내부에 엔터 입력시
// == submit (제출)
changePw?.addEventListener("submit", e=>{

  // e.preventDefault(); // 기본 이벤트 막기
  // -> form의 기본 이벤트인 제출 막기
  // -> 유효상 검사 조건이 만족되지 않을때 수행

  // 입력요새 모두 얻어오기
  const currentPw = document.querySelector("#currentPw");
  const newPw = document.querySelector("#newPw");
  const newConfirm = document.querySelector
