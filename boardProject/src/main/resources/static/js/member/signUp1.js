...
/* 필수 입력 항목의 유효성 검사여부를 체크하기위한 객체(체크리스트) */
const checkObj = {
  "memberEmail" : false,
  "memberPw" : false,
...
emailMessageObj.normal = "메일을 받을 수 있는 이메일은 입력해주세요";
...
memberEmail.addEventListener("input", e=>{
  ...
  .then(count => {
    // 매개변수 count : 첫번째 then에서 return된 값이 저장된 변수
...
memberNickname.addEventListener("input", ()=>{
  ...
  .then(count => {
    ...
    checkObj.memberNickname = true; // 유효한 닉네임을 기록
...
const initMin = 4; // 초기값 5분에 1초 감소된후 분
const initSec = 59; // 초기값 5분에 1초 감소된후 초
...
/* 회원 가입 form제출시 전체 유효성 검사 여부 확인 */
const signUpForm = document.querySelector("#signUpForm");

signUpForm.addEventListener("submit", e=>{

  // e.preventDefault(); // 기본 이벤트(인 form 제출)막기
