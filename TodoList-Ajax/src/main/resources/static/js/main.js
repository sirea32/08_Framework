...
addBtn.addEventListener("click", () => {
  ...
  // - body : POST/PUT/DELETE 요청시 전달한 데이터

  fetch("/todo/add", {   // 지정된 주소로 비동기 요청(ajax)
      method : "POST",  // 데이터 전달 방식을 POST로 지정
      headers: {"Content-Type": "application/json"}, // 요청 데이터의 형식을 JSON으로 지정      body : JSON.stringify( {K:V, K:V} ) // JS객체를 JSON 형태로 변환하여 Body에 추가
...
function getTotalCount(){
  ...
  /* GET 방식 fetch() */
  fetch("/todo/totalCount") // 비동기 요청 수행 -> Promise(비동기 결과를 가지고오는) 객체 반환
  .then(response => {
  ...
  // 첫 번째 then에서 반환된 값을 매개변수에 저장한 후
  // 수행되는 구문
  .then(totalCount => {
...
function getCompleteCount(){
  ...
  /* *첫번째 then
    ...
    - 매개변수 response : 
        -> 응답결과, HTTP 상태 코드, 요청주소 등이 담겨 있는 객체

     *두번째 then
    - 비동기 요청으로 얻어온 값을 이용해서 수행될 JS코드 작성하는 구문
   */
  fetch("/todo/completeCount") // 비동기 요청해서 결과 데이터 응답 받아오겠다/가져오겠다
  .then(response => {
    if(response.ok){ // 비동기 통신 성공시(HTTP 상태코드 200)
...
function selectTodoList(){
  ...
  .then(result => {
    ...
    /* #tbody 내용을 모든 지운후 조회된 내용의 요소 추가 */
