...
public class ExampleController {
	...
	// 1) page				-> 현재 페이지(Servlet/JSP)
	
	// 2) request			-> 요청 받은 곳(Servlet/JSP) + 요청 위임(forward)(Servlet/JSP) 받은 곳
	
	// 3) session			-> 클라이언트가 서버 최초 연결시 생성,
	//									 연결한 브라우저 종료 또는 세션 시간 만료까지 유지가 된다
	
	// 4) application -> 서버 실행시 1개만 생성
	//									 서버가 종료될때까지 유지
	...
	public String ex1(HttpServletRequest req, Model model) {
		...
		// DTO 객체를 만들어 Model에 세팅 + 빌터 패턴 사용
		Student std = Student.builder()
												 .studentNo("1111")
												 .name("짱구")
												 .age(15)
												 .build();
		// -> 필드 전체가 아닌 일부 초기화시 활용도가 좋음
		log.debug("std : {}", std);
	...
	public String ex5(Model model) {
		...
		model.addAttribute("message", "서버에 전달된 메시지");
		model.addAttribute("num", 12345);
		
		Student std = new Student();
		std.setStudentNo("6789");
		model.addAttribute("std", std);
//		model.addAttribute("std100", null);
